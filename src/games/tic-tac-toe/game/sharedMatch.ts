...
/**
 * Controlador de UN tablero de sala (modo sala, PvP). Misma sincronizacion que
 ...
 * porque cada ronda reparte a todos los jugadores en duelos 1v1 simultaneos.
 */
export interface SharedMatchOpts {
  /** Fila de room_match_state de este tablero (una por pareja en la ronda). */
  boardNo: number;
  /** Los dos asientos [X, O] de esta pareja. */
  seats: [string, string];
  /**
...
export class SharedMatch {
  ...
  /** Puntaje de la ronda (y parcial por timeout): 1 si gane, si no 0. */
  myScore(): number {
  ...
  private async boot(): Promise<void> {
    ...
    for (;;) {
      ...
      const row = await fetchMatchState<TttMatchState>(
        this.room.code,
        this.room.round(),
        this.boardNo,
      );
      ...
      if ((this.room.isHost() || Date.now() - since > CREATE_FALLBACK_MS) && !this.spectate) {
        const init: TttMatchState = { ...createState(0), players: this.seats, seq: 0 };
  ...
  private async maybeMoveAfk(): Promise<void> {
    ...
    this.version = expected + 1;
    this.lastAnimSeq = next.seq;
    if (!this.passive) SoundEffects.playPlace(state.turn);
    this.render();
    this.checkFinish();
    this.queueWrite(next, expected);
  }
