...
entre arbitro y relay) **y Neon Drift** (`car-race`, **relay** de
...
- `src/protocol.ts` — tipos de mensajes (por juego: `wb:*` de Bomba Palabra,
  ...
  `tc:*` de Telefono Cortado, `pt:*` de Manchon, `mt:*` de Imitame, `hp:*` de Papa Caliente, `dr:*` de Derrumbe). **Se duplican en el cliente**
...
Impostor, Telefono Cortado, Manchon, Imitame, Papa Caliente y Derrumbe **no** pueden: existen por el server (en Bomba/Cadena porque el
...
ser secreta y solo el server puede guardarla; en Derrumbe por lo mismo que en Manchon, con el piso), asi
...
- `resolveGameServerUrl()` — la URL a la que conectarse, con caida al respaldo. **La
  llaman los once juegos server-side** en su `connect()` (`word-bomb`, `word-chain`,
  `basta`, `impostor`, `telefono-cortado`, `pong`, `car-race`, `paint-turf`, `imitame`,
  `hot-potato`, `derrumbe`), que por eso es `async` y
