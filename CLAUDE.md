...
las fases y hace de relay de las tomas de voz), **Derrumbe** (`derrumbe`, TNT Run: el **piso** lo
arbitra el server y el **movimiento** lo simula cada cliente, un hibrido entre arbitro y relay)
...
- `src/protocol.ts` — tipos de mensajes (por juego: `wb:*` de Bomba Palabra,
  ...
  `tc:*` de Telefono Cortado, `pt:*` de Manchon, `mt:*` de Imitame). **Se duplican en el cliente**
...
Impostor, Telefono Cortado, Manchon, Imitame y Derrumbe **no** pueden: existen por el server (en Bomba/Cadena porque el
...
el server corre las fases y reparte las tomas de audio; en Derrumbe por lo mismo que en Manchon, con el piso), asi
...
- `resolveGameServerUrl()` — la URL a la que conectarse, con caida al respaldo. **La
  llaman los diez juegos server-side** en su `connect()` (`word-bomb`, `word-chain`,
  `basta`, `impostor`, `telefono-cortado`, `pong`, `car-race`, `paint-turf`, `imitame`, `derrumbe`), que por eso es `async` y
