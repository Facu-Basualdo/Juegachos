...
Monorepo of small browser minigames (Neon Cylinder, Flappy Bird, Stack Tower, Rhythm Tap, Jump Ball, Reaction Time, City Bloxx, Sliding Puzzle, Asteroids, Mini Frogger, Neon Drift, Odd One Out, Dunk Shot, Memoria, Kunai Throw, Keepers!, Western Shoot, Barra Libre, Crono Ciego, El Trile, PONG, Block Paddle, Simon, Topos, Snake, Ta-Te-Ti, Conecta 4, Mecano, Final Sentence, Neon Sawblades, Space Rush, Lights Out, Boilerbound, Timber!, Puerco Araña, Circuit Breaker, Ring Runner, Pulso de Acero, Memoria de Color, Al Centro, Bomba Palabra, Cadena de Palabras, Hole in None, Cannon Dodge, Pizza Express, Danger Wings, Click the Number, Macaco Tilt, Templo Rodante, Basta, Impostor, Manchon, Imitame, Birome, Papa Caliente, Derrumbe, Luz Roja, Luz Verde and Pista Loca), each independently playable — except Bomba Palabra, Cadena de Palabras, Basta, Impostor, Manchon, La Escalera, Imitame, Papa Caliente, Derrumbe, Luz Roja, Luz Verde and Pista Loca), which are **rooms-only** (they need a multiplayer room and the game server; see "Game server" below) — plus a landing page to pick one. (Rocket SpaceX / `rocket-arena`, Telefono Cortado / `telefono-cortado` and Patas Largas / `patas-largas` still live in the repo but are hidden from the roster via `hidden: true` in their `meta.ts`: rocket-arena due to errors, Telefono Cortado because it was pulled from the roster, Patas Largas because its walk is not playable yet — its simulation work and findings are deliberately kept on main, see its `CLAUDE.md`. Telefono Cortado is also rooms-only and server-backed; their code and their game-server sims stay in place.) Stack: Vite + TypeScript, no framework. Deployed as a static site (Vercel), plus a separate Node game server (`server/`) on Railway for the real-time / server-authoritative games.
...
- `src/games/telefonocortado.ts` — `TelefonoCortadoSim`: telefono descompuesto con dibujos.
  ...
  state se leeria desde las devtools. Las cadenas del reveal se difunden **de a una**
