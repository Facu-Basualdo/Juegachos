...
**y Neon Drift** (`car-race`, **relay** de
...
- `src/protocol.ts` — tipos de mensajes (por juego: `wb:*` de Bomba Palabra,
  ...
  `tc:*` de Telefono Cortado, `pt:*` de Manchon, `mt:*` de Imitame, `hp:*` de Papa Caliente, `dr:*` de Derrumbe, `lr:*` de Luz Roja, Luz Verde, `pl:*` de Pista Loca). **Se duplican en el cliente**
...
Impostor, Telefono Cortado, Manchon, Imitame, Papa Caliente, Derrumbe, Luz Roja y Pista Loca **no** pueden: existen por el server (en Bomba/Cadena porque el
...
y el ritmo tienen que ser los mismos para todos), asi
...
- `resolveGameServerUrl()` — la URL a la que conectarse, con caida al respaldo. **La
  llaman los trece juegos server-side** en su `connect()` (`word-bomb`, `word-chain`,
  `basta`, `impostor`, `telefono-cortado`, `pong`, `car-race`, `paint-turf`, `imitame`,
  `hot-potato`, `derrumbe`, `luz-roja`, `pista-loca`), que por eso es `async` y
