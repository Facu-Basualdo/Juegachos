...
adivinanzas) **y Neon Drift** (`car-race`, **relay** de posiciones — el unico que no usa
el server para arbitrar nada, ver abajo).
...
Impostor y Telefono Cortado **no** pueden: existen por el server (en Bomba/Cadena porque el
...
unico que conoce las frases secretas y valida las adivinanzas), asi que sin
`VITE_GAME_SERVER_URL` muestran "no disponible" (excepcion deliberada y documentada a la
...
- `resolveGameServerUrl()` — la URL a la que conectarse, con caida al respaldo. **La
  llaman los siete juegos server-side** en su `connect()` (`word-bomb`, `word-chain`,
  `basta`, `impostor`, `telefono-cortado`, `pong`, `car-race`), que por eso es `async` y
