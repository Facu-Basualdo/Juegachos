...
entre arbitro y relay), **Pista Loca** (`pista-loca`, Block Party: el server arma el dibujo de
...
- `src/protocol.ts` — tipos de mensajes (por juego: `wb:*` de Bomba Palabra,
  ...
  `tc:*` de Telefono Cortado, `pt:*` de Manchon, `mt:*` de Imitame, `hp:*` de Papa Caliente, `dr:*` de Derrumbe, `pl:*` de Pista Loca). **Se duplican en el cliente**
...
Impostor, Telefono Cortado, Manchon, Imitame, Papa Caliente, Derrumbe y Pista Loca **no** pueden: existen por el server (en Bomba/Cadena porque el
...
ser secreta y solo el server puede guardarla; en Derrumbe por lo mismo que en Manchon, con el piso; en Pista Loca porque el dibujo, el color
...
- `resolveGameServerUrl()` — la URL a la que conectarse, con caida al respaldo. **La
  ...
  `hot-potato`, `derrumbe`, `pista-loca`), que por eso es `async` y
