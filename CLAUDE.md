...
- **Antes de simular cualquier cosa, consultar [SIMULATION_ARCHITECTURE.md](SIMULATION_ARCHITECTURE.md) (en la raiz, junto a este archivo: `docs/` esta gitignoreado y ahi el documento no le llegaria a nadie mas) — y PEDIR PERMISO antes de aplicar el metodo.** Ese documento es la referencia del proyecto para elegir metodo de simulacion, fisica e IA (integradores, PID, IK, pendulo invertido/ZMP, behavior trees, HFSM, boids, A*, minimax/MCTS), con una matriz por arquetipo de minijuego y los antipatrones vetados. El flujo es: (1) identificar el arquetipo del juego en la matriz, (2) proponerle al programador el metodo que corresponde y por que, (3) **esperar su OK antes de implementarlo**. No arrancar a codear la simulacion por las tuyas aunque el metodo parezca obvio: la eleccion condiciona toda la arquitectura del juego y es una decision del programador, no del asistente.
...
- `src/games.ts` — the `GameEntry` type (`id`, `title`, `description`, `path`, optional `accent`, optional `controls`, `category`, optional `order`, `added`, `mobile`, optional `hidden`, optional `roomsHidden`, optional `roomTimeLimitSec`) plus `coverUrl`. `roomTimeLimitSec` is the round's time limit **in rooms only**, in seconds (see "Salas"); omit it and the round is untimed. `added` is the ISO `YYYY-MM-DD` date the game entered the roster and is **required**: `order` is curated by hand and does not track when a game was added, so the landing's default "Nuevos" sort reads `added` instead. `mobile` (boolean) is whether the game can be played on a phone and is **also required** — deliberately not optional, so a new game can't default into being advertised as mobile-ready without anyone having tried it on a phone (see "Jugable en celular" below for what earns a `true`). The landing shows **platform chips** next to the category chip on each card (the row sits at the **bottom** of the cover: the top corners are taken by the favorite heart on the left and the "Ranking" button on the right): a monitor icon on **every** card (the whole roster plays on a computer) plus a phone icon on the games that declare `mobile: true`. They are icons and not the words "PC" / "Móvil" because next to the category chip two text chips read as two categories. Both SVGs are inline in `src/main.ts` (`PC_BADGE` / `MOBILE_BADGE`, no loose assets) and take their color from `currentColor`, so the accent lives in one CSS rule: the PC one stays in the chip's base cream and only the phone is cyan (`.game-card__tag--mobile`), since that is the one carrying information — the pair reads "también en el celu". Shared icon-chip styling is `.game-card__tag--icon`. The row is `.game-card__tags`. Accessibility: the `<svg>` is `aria-hidden` and the `aria-label` lives on the wrapping span, otherwise the chip would be invisible to a screen reader. `controls` is a one-line "how to play" string shown in the room-mode briefing before each round (see "Salas"); omit it and the briefing just skips the controls block. `roomsHidden` drops a game from rooms only (kept on the landing) — the file also exports `roomGames` (= `games` minus `roomsHidden`) for the room selection/vote/random/picker paths. It does **not** list the games by hand: it auto-discovers them with `import.meta.glob("./games/*/meta.ts", { eager: true })`, filters out `hidden` ones, and sorts by `order` (ascending; entries without `order` fall to the end, then alphabetical by title). **This file is closed for modification** — adding a game means adding its `meta.ts`, never editing `games.ts` (this is what stops the registry from producing merge conflicts on every new game).
- `src/games/<id>/meta.ts` — that game's registry entry: `export const meta: GameEntry = { ... }`. **Every game needs this file** (it is what the landing page, rooms, and random-game picker read). Set `order` to place the card (existing games use multiples of 10). Set `hidden: true` to pull a game from the roster without deleting it (`rocket-arena`, hidden due to errors; `patas-largas`, not playable yet) — its code stays in the repo and it just disappears from the landing and the rooms voting / random pool. Set `roomsHidden: true` to exclude a game from **rooms only** (host picker, vote pool, random and playlist) while keeping it on the landing (no game currently uses it — Pong used to, before it gained its server-authoritative room mode); `src/games.ts` derives `roomGames` (= `games` minus `roomsHidden`) for that, and the room code uses `roomGames` for selection but still looks games up by id in `games` so an in-flight round resolves its title/URL.
...
3. Create `src/games/<id>/meta.ts` exporting `const meta: GameEntry` (see `src/games/blind-time/meta.ts`). Give it an `order` (multiples of 10) to place its card, an `added` with today's date (`YYYY-MM-DD`, required — it drives the landing's default "Nuevos" sort) and a `mobile` boolean (also required — **probalo en un celular antes de poner `true`**, ver el paso 6). **Do not touch `src/games.ts`** — the glob picks the new `meta.ts` up automatically.
...
6. Implement the mandatory Enter-to-start 3 / 2 / 1 / YA countdown (see "Shared UX pattern" below). **El arranque tambien tiene que entrar por un toque**, y el listener va sobre el `container` o el overlay — nunca sobre el canvas, que la pantalla de inicio tapa (ver "El toque de arranque no puede colgar del canvas"). Eso mas el gameplay sin teclado fisico es lo que decide el `mobile` del paso 3.
...
Every game reports its final score to a shared global leaderboard backed by **Supabase** (Postgres + client SDK called straight from the browser with the anon key and Row Level Security — no server code). There are two boards per game — **"Este mes"** (calendar month, Argentina time) and **"Historico"** — both derived from a **history of every finished run** (see "Mensual, historial y salas" below). Lives in `src/shared/`:
...
- `scoring.ts` — builds `GAME_SCORING[gameId]` by auto-discovering each game's `export const scoring: GameScoring` via `import.meta.glob("../games/*/meta.ts", { eager: true })` (same Open/Closed pattern as `src/games.ts` — **do not edit this file to add a game**), and exposes the accessors `getScoring` / `getDirection` / `formatScore`. It also re-exports everything from `scoring-core.ts` so existing `import { ... } from ".../shared/scoring"` keep working. A `GameScoring` has `direction` (`"higher"` = bigger is better, default; `"lower"` for reaction-time, blind-time, sliding-puzzle, car-race), optional `format`, optional `variants` (independent boards, e.g. sliding-puzzle sizes), and optional per-variant overrides `variantDirection` / `variantFormat` (used when one variant sorts/formats differently from the base — e.g. memory-match is `"higher"` for room-mode pairs but its solo `"solo"` board is `"lower"`). Two more optional fields decide what the global board measures and what feeds it: `ranking` (`"best"` default = each player's best score; `"wins"` = number of room matches won) and `roomRanked` (default `true`; `false` keeps the game's finished room runs off the global board) — see "Mensual, historial y salas" below; `getRankingMetric` / `isRoomRanked` read them. A game whose scoring is exactly the default `{ direction: "higher" }` **omits** `scoring` from its `meta.ts` (the `getScoring` default covers it); every game with non-default scoring must declare it there.
...
- **Favoritos** (landing only, no Supabase): a heart button on the top-left corner of each cover (`.game-card__fav`) pins that game to the head of the grid. Stored in `localStorage` `mg:favorites` as an id array ordered **most recently marked first**, which is the order they show in — marking a game puts it above the previously marked ones. No cap on how many. They are pinned in **every** sort mode (`orderedGames()` = favorites first, then `sortedGames()` for the rest) on purpose: a fourth "Favoritos" entry inside the sort dropdown would go unnoticed. Read back through a whitelist against the current roster, so a favorite of a game that was removed or turned `hidden` is dropped instead of lingering. Two gotchas baked into the code: (a) the card is an `<a>`, so the heart's click **must** `preventDefault` + `stopPropagation` like the "Ranking" button does — otherwise marking a favorite would open the game *and* add a play to the "Populares" count; (b) the heart is always visible, never revealed on `:hover` (on a phone there is no hover and it would be unreachable). It is a `<button>` with `aria-pressed`, not a decorative icon.
- **La grilla se "asienta" al primer reordenamiento (`grid.is-settled`).** Reordering re-inserts the card nodes, which restarts each card's `rise` entry animation — with its `--i` stagger that is several seconds of the whole grid fading back in. Harmless when it only happened on a sort change, unacceptable now that a heart click reorders. So `applyOrder()` adds `is-settled` to the grid, whose CSS rule kills the animation; the entry animation belongs to the page load and nothing else.
- `leaderboard.ts` — `submitScore(gameId, score, { variant, source, place, players })` records one finished run in the history; `fetchTop(gameId, { variant, limit, period })` returns the Top N with **one row per player** (their best, or their win count for `ranking: "wins"` games), `period` = `"all"` (default) or `"month"`. `monthStart()` / `monthLabel()` compute the current month in Argentina time (UTC-3 fixed, no DST since 2009) on the client, so the monthly board is the same whoever looks at it. `submitScoreIfTop` (circuit-breaker's per-level marks) checks against the **monthly** board, the wider filter. Reads go through the `leaderboard_best` / `leaderboard_wins` RPCs; see the migration note below for a DB without them.
- `LeaderboardPanel.ts` — self-contained DOM component (injects its own CSS once), with two tabs **"Este mes"** (default) / **"Historico"** (`.mg-lb__tab`, `aria-pressed`; the landing's light modal restyles them in `src/style.css`). On game over, when a nickname is already saved (`getNickname()`), the run is **always recorded without prompting** — it is the history, it enters whether or not it makes the Top. Without a saved name, it shows the name form only if the score qualifies for the **monthly** Top 10 (that is when it is worth interrupting the player); otherwise the run is not recorded (no name, no history). Then shows the Top 10 of the selected tab and highlights the player's own row. For `ranking: "wins"` games the title reads "Victorias en salas" and values are `formatWins` ("3 victorias"). Reused by each game's game-over overlay and by the landing modal. The name itself can be set/edited on the landing (a "Tu nombre" field in the hero, between the title and the search box).
- `leaders.ts` — **Salón de la fama**: a ranking of the players who lead (are #1 in) the global leaderboard of the **most games**. It is **derived live** from the `scores` table — **no dedicated table, no persistence, nothing to do with rooms**. `fetchGameLeaders()` reads all scores (paginated past Supabase's 1000-row cap), and for each game in the roster picks its current leader on the representative board (variant `variants[0]`, direction-aware — exactly the board each landing card shows as its champion) (for `ranking: "wins"` games, the player with the most room wins — each board carries a `wins` flag to the `game_leaders` RPC) and tallies how many games each player leads; it returns `{ ranking: {player, games}[], totalGames }` where `totalGames` is how many games currently have a leader. Degrades with the rest: no credentials -> empty. **UI:** the podium lives on its own page `/fame/` (`fame/index.html` + `src/fame/main.ts`, a non-game entry like `/rooms/`, registered in `vite.config.ts`): topbar + a dark gold-accented `.fame` plaque (a stats row `N juegos con líder / M líderes`, podium for the top 3 with the leader centered + elevated, ranked list for 4th onward, empty state when there are no scores) + footer, reusing the landing's `src/style.css`. The landing (`src/main.ts`) only shows a compact `.fame-banner` (styled like the Salas banner, gold accent, with a mini top-3 preview of initials) that links to `/fame/`; both appear only when the leaderboard is enabled.
...
- **Migration.** The history columns and the `leaderboard_best` / `leaderboard_wins` RPCs live in `supabase/schema.sql` (idempotent; re-run it after pulling). Against a DB that does not have them yet the client still works: `leaderboard.ts` warns **once** in the console, remembers it per page (`rpcMissing`; the first RPC call is a probe the rest wait for — the landing asks ~60 champions in parallel and each one used to 404), falls back to a plain query deduped per player in the browser, inserts without the new columns, and skips win rows (they have nowhere to keep the placement). Win boards are simply empty until the migration runs.

### Mensual, historial y salas

- **`scores` is the history.** It used to hold only the scores that made the Top 10 at that moment; now every finished run of a named player is a row (`source` = `'solo'` / `'room'`, plus `place` / `players` for room matches). There is no separate players table: "who plays" is the history itself (`select distinct player`, first/last `created_at`, runs per game). The rankings are derived from it, never stored.
- **One row per player.** Once every run is saved, a single grinder would fill the whole Top 10 with themselves, so `leaderboard_best` keeps each player's best (`distinct on (player)`), with the same tiebreak as before (earlier `created_at`, then `id`).
- **Monthly = since the 1st at 00:00 Argentina time**, computed client-side (`monthStart()`) and passed as `p_since`; the all-time board passes `null`. The landing card champion and the Salon de la fama stay all-time.
- **Room runs count, but only finished ones.** `RoomMode.recordRanking` records the run when the game calls `reportScore` (the finished path) and the room write succeeded for the first time — `reported` is restored from the DB on reload, so an F5 does not duplicate it. Timeout partials (`finished: false`) never count: they are not comparable. Games whose room mode measures something else than solo declare `roomRanked: false`: `connect-four` / `tic-tac-toe` (1v1 duel 1/0 vs a streak against the AI), `pong` (PvP vs AI), `whack-a-mole` (cut by the room clock), `flash-math` / `shell-game` (a different room score), `memory-match` (shared board pairs). A game with `variants` must say which board the run goes to (`reportScore(score, { variant })`: car-race passes the track, tower-of-hanoi the discs, number-memory the mode, typing-race `"final"`, circuit-breaker `"general"`; sliding-puzzle / lights-out / click-the-number get it from `ROOM_VARIANTS`); without it the run is not recorded — a board with numbers that don't compare is worse than a run that didn't count. `reportScore(score, { ranked: false })` skips one specific run (paint-turf and marea-lava use it for their "no connection" zero; marea-lava also for a player without a seat).
- **Rooms-only games: wins or best score.** The ones whose room score is a placement (`jugadores - puesto`, which depends on how many played) rank by **room wins** — `ranking: "wins"`: `word-bomb`, `word-chain`, `basta`, `impostor`, `imitame`, `hot-potato`, `telefono-cortado`. Their game-over passes `{ place, players }` (or `{ ranked: false }` for someone missing from the server ranking, e.g. joined late); a win is `place = 1` with `players >= 2`, so a one-player room does not farm wins. The rooms-only games with a real score (`derrumbe` seconds, `luz-roja`, `pista-loca` rounds, `paint-turf` cells, `marea-lava` height, `la-cuerda` metres) keep the default best-score board; they don't mount a `LeaderboardPanel`, so their board is seen from the landing card's "Ranking" button.
...
- **Host-authoritative**: only the host writes phase transitions (start/close round, open vote, finish); players write only their own rows. Points are computed client-side by the pure functions in `points.ts`. If the host disappears, another player takes over **automatically** — see "Que un jugador se desconecte no puede trabar la sala" below.
- Same spoofable trust level as `scores` (anon key + open RLS policies) — accepted and documented in the SQL.

Files in `src/shared/room/`: `types.ts` (types + settings constants), `api.ts` (CRUD, no-op without credentials), `channel.ts` (`RoomChannel`; besides presence and the `sync` ping it carries an ephemeral **`live`** broadcast — `broadcastLive(data)` / `onLive(cb)`, exposed on `RoomMode` — for cosmetic per-frame-ish state that must **not** touch the DB: a lost packet is simply lost. La Escalera uses it ~8 times a second to show each rival running on their own escalator, and Dalgona 4 times a second to draw every rival's cookie live — carving as one character per segment, needle, crack segment; it is never a source of truth for scores), `points.ts` (`rankRound` / `computeTotals`, pure), `RoomOverlay.ts` (self-contained fixed full-screen DOM overlay: waiting / results / voting / final + top strip; styled to match the landing — cream card, ink borders, pill buttons — with the palette hardcoded since it injects into each game page. The top strip (`setStrip(text, lights)`) shows the room code / round / clock plus a row of player lights — one dot per registered player, **green** alive, **red** dead/finished, **gray** disconnected; `roomMode.stripLights()` derives them each tick from `presentPlayers()` + the round's reported scores, own dot ringed), `roomMode.ts` (orchestrator + the per-game contract), `matchState.ts` (generic shared-board match state, see below), `roomRun.ts` (see "Sobrevivir un F5" below).

**Que un jugador se desconecte no puede trabar la sala.** La sala es host-autoritativa, asi que **el que se va importa muchisimo si era el host**: nadie mas escribe las transiciones de fase (cerrar la ronda, cerrar el briefing, cerrar la votacion) ni destraba los tableros compartidos, y la sala queda congelada para todos. Tres candados, y los tres hacen falta:

1. **Migracion de host automatica (`roomMode.maybeTakeOverHost`).** Si el host no aparece en `presentPlayers()` durante `HOST_ABSENT_MS` (20s) seguidos, otro jugador toma el control **solo**, sin que nadie apriete nada, en **cualquier** fase (incluida `playing`). Antes esto era un boton manual (`RoomOverlay.offerTakeover`, ya eliminado) y **no servia**: solo se ofrecia en fases "estables" — durante `playing` exigia haber reportado el puntaje — y ademas `offerTakeover` no dibuja nada con el overlay oculto, que es exactamente como esta la pantalla mientras se juega. O sea que en el caso mas comun (el host se va en plena ronda) el boton no llegaba a existir y la sala se moria ahi. El candidato es **deterministico**: el primero de `presentPlayers()` (orden `joined_at`) que no sea el host ausente, con un escalonado de `TAKEOVER_STAGGER_MS` (5s) por posicion — asi dos no escriben a la vez y, si el primero tiene la pestana en segundo plano (el navegador le estrangula los timers), sigue el segundo. Con la lista de presentes vacia (canal recien suscripto, o caido) **nadie** se autoproclama: la condicion pide estar uno mismo en la lista.
2. **El tablero compartido no lo crea solo el host (`CREATE_FALLBACK_MS`, en `matchState.ts`).** Los cinco juegos que crean una fila en `room_match_state` (Memoria, Conecta 4, Ta-Te-Ti, El Trile y la votacion de discos de Hanoi) la creaban **solo** el host: si el host se desconectaba antes de cargar la pagina de la ronda, todos se quedaban mirando "Preparando el tablero..." / "Esperando un rival..." hasta que la migracion de host apareciera. Ahora, pasados 6s sin fila, la crea cualquier jugador; el insert ya era idempotente por PK (gana el primero, el resto relee), asi que la carrera estaba contemplada de antes. En El Trile eso ademas obligo a agregar un reintento (`waitForRoomState`): su `syncRoomState` se disparaba **solo** con el broadcast "sync", que en ese caso no llegaba nunca.
3. **Al desconectado no se le espera el turno (`presentPlayers()` en el contrato de `RoomMode`).** En los juegos por turnos el jugador que se fue sigue en el orden de turnos (su fila de `room_players` es durable a proposito, para que pueda volver), asi que le tocaba jugar y frenaba a todos hasta que venciera la ventana AFK — 20-25s **por vuelta de turnos**, que con dos caidos deja el tablero practicamente parado (este es el sintoma que se reporto en Memoria). `RoomMode` expone ahora `presentPlayers()` (los registrados conectados, en el orden de `players()`, igual para todos los clientes) y cada juego por turnos usa una ventana corta cuando el de turno esta **desconectado**: `AFK_ABSENT_SKIP_MS` (3s) en Memoria, que ademas saltea de una a todos los ausentes seguidos en una sola escritura, y `AFK_ABSENT_MOVE_MS` (5s) en Conecta 4 / Ta-Te-Ti. Ojo con la lista vacia: si `presentPlayers()` no tiene a nadie (canal aun sin sincronizar) **no** se asume que se fueron todos, se cae a la ventana AFK de siempre.

Ademas, en Conecta 4 y Ta-Te-Ti el destrabe de turno dejo de ser exclusivo del host: **tambien lo hace el rival del jugador parado en su propio tablero** (nunca el propio turno, salvo el host). Eran los dos juegos donde un host caido congelaba duelos que no lo involucraban, porque los tableros ajenos los administra el host con instancias `passive` que se crean al arrancar la ronda — un host nuevo que toma el control a mitad de ronda no las tiene.

Lo que **no** cubre nada de esto: un jugador presente pero inactivo (pestana abierta y sin tocar nada) no es un desconectado — para eso esta `roomTimeLimitSec` y las ventanas AFK normales.
...
- In the game-over handler: `if (this.room) this.room.reportScore(score); else this.hud.showRanking(...)`. `RoomMode` records a **finished** room run in the global history / ranking by itself (timeout partials never count) — the game never calls `showRanking` in a room. See "Mensual, historial y salas" for `roomRanked`, `{ variant }` and `{ place, players }`.
...
- `onStart` (optional) is fired **once** by `RoomMode` when the round becomes `playing`, so every player's run auto-starts together instead of each one pressing Enter — normally `() => this.beginCountdown()` (mini-frogger used to be the exception with no countdown and passed `() => this.start()`; it has the countdown now, so there is no exception left). Games that drive their own room start (car-race calls `beginCountdown()` in its `boot()`; rocket-arena is real-time) don't pass it.
...
Some room games stream each player's live position over their own **ephemeral broadcast channel** — no DB, one channel per room+round, separate from the `RoomChannel` so the high-frequency traffic doesn't mix with the room sync. Each game owns its copy (per the decoupling rule): `car-race`'s `RaceChannel`, `cannon-dodge`'s `DodgeChannel`, `templo-rodante`'s `TempleChannel`, `rocket-arena`'s `ArenaChannel`, `typing-race`'s `TypingChannel`, `monopoly-mundial`'s `MonopolyChannel`, `birome`'s `InkChannel`. Copy `DodgeChannel.ts` when you add another one — it, `templo-rodante`'s `TempleChannel`, `birome`'s `InkChannel` and `car-race`'s `RaceChannel` are the only ones that get the two rules below right.

**Antes de elegir la cadencia, preguntate si hace falta mandar POSICIONES.** `templo-rodante` es el caso testigo: en vez del snapshot de cada cuadro manda el **evento de pose** (`jump` / `duck` / `stand` / `dead`) y el receptor corre la misma parabola con las mismas constantes de su `Runner`, que es la misma clase que usa el jugador local. La animacion sale identica y el trafico baja de 10/s por jugador a ~3/s. Cuando el estado del rival se puede **derivar** de un evento discreto, mandarlo entero cuadro a cuadro es tirar presupuesto de canal a la basura. `birome` lleva la idea al extremo y es **el molde para un juego de sala que no se sienta con lag** (lo opuesto a Manchon): cada uno corre su partida local sobre un recorrido sembrado, la pendiente fija (+-45) hace que el trazo quede definido solo por los puntos donde el jugador apreto o solto, y la velocidad depende de la distancia y no del tiempo, asi que el receptor adelanta al rival sin sincronizar relojes. A los rivales se los dibuja 0.2 s en el pasado y la linea sale exacta (medido: 0 px de error hasta 250 + 150 ms de jitter). Ver su `CLAUDE.md`. Dos reglas que trae aparejadas: el keepalive **no** puede reenviar el evento que arranca una animacion (reenviar `jump` reiniciaria el salto del rival, por eso solo reafirma `stand`/`duck`/`dead`), y el receptor no puede dejar que un keepalive corte una animacion en curso (un `stand` que llega a mitad del salto se aplica al aterrizar).
...
| `derrumbe` | 8 x 20/s = **160/s** de subida (+ 20 snapshots/s de bajada por sala) | **Nacio en el server**: el piso es estado compartido y la cadencia tambien pasa el tope. |
...
pistas por turno y votacion, sin diccionario), **Telefono Cortado** (`telefono-cortado`,
telefono descompuesto con dibujos: el server guarda las frases secretas y valida las
adivinanzas), **Manchon** (`paint-turf`, captura de territorio en tiempo real: el tablero es
estado compartido y lo arbitra el server), **Imitame** (`imitame`, clon de Mimic Party: arbitra
las fases y hace de relay de las tomas de voz), **Papa Caliente** (`hot-potato`, la mecha secreta
vive solo en el server, que ademas compensa la latencia de cada pase), **Derrumbe** (`derrumbe`,
TNT Run: el **piso** lo arbitra el server y el **movimiento** lo simula cada cliente, un hibrido
entre arbitro y relay), **Luz Roja, Luz Verde** (`luz-roja`, el server es duenio del semaforo y
del reloj; cada cliente juzga su propia eliminacion en rojo para no castigar la latencia),
**Pista Loca** (`pista-loca`, Block Party: el server arma el dibujo de
cada ronda y decide la musica, el color y el tiempo; cada cliente juzga su propia caida),
**Marea de Lava** (`marea-lava`, pared de plataformas con la lava subiendo: el server manda la
semilla de la torre y lleva el reloj de la lava), **La Cuerda** (`la-cuerda`, puente angosto con dos
cuerdas gigantes que aceleran: el server lleva el reloj del que sale el angulo de cada cuerda y
resuelve los empujones; cada cliente juzga su propio golpe) **y Neon Drift** (`car-race`, **relay** de
...
- `src/protocol.ts` — tipos de mensajes (por juego: `wb:*` de Bomba Palabra,
  ...
  `tc:*` de Telefono Cortado, `pt:*` de Manchon, `mt:*` de Imitame, `hp:*` de Papa Caliente, `dr:*` de Derrumbe, `lr:*` de Luz Roja, Luz Verde, `pl:*` de Pista Loca, `ml:*` de Marea de Lava, `lc:*` de La Cuerda). **Se duplican en el cliente**
...
- `src/games/telefonocortado.ts` — `TelefonoCortadoSim`: telefono descompuesto con dibujos.
  ...
  del de dos atras, asi con 3+ jugadores nunca te toca lo tuyo) y **valida las adivinanzas
  server-side**: la frase a adivinar viaja SOLO por el evento dirigido `tc:you` (con la pista
  tipo ahorcado, letras ocultas como "_"), nunca en el broadcast `tc:state`, porque en el
  state se leeria desde las devtools. La adivinanza es **tolerante** (ignora articulos,
  acentos y espacios, y perdona errores de tipeo en proporcion al largo) y avisa si
  estuvo cerca. **Con 1-2 jugadores no hay fase de escritura**: las frases salen del banco
  del server, porque con dos el unico que podria adivinar una frase es el que la escribio.
  Las cadenas del reveal se difunden **de a una** (`tc:chain`): juntar 8 dibujos en un
  mensaje se acerca al `maxHttpBufferSize` de socket.io. Estado scopeado por **ronda**
  (`round` en el `tc:join`), igual que Neon Drift. Un partido es una vuelta completa. Ver
  el `CLAUDE.md` de `telefono-cortado`.
- `src/games/paintturf.ts` — `PaintTurfSim`: Manchon. Captura de territorio en
  ...
  su posicion — declararla, como hace la paleta de PONG, dejaria teletransportarse y
  pintar todo el tablero desde las devtools. La grilla entera viaja solo en el
  `pt:init` del join/reconexion (un caracter por celda); los snapshots llevan
  unicamente las celdas cambiadas. Estado scopeado por **ronda** (`round` en el
  `pt:join`), igual que Neon Drift. Ver el `CLAUDE.md` de `paint-turf`.
- `src/games/imitame.ts` — `ImitameSim`: Imitame (clon de Mimic Party). Sortea el sonido
  (solo el id: la definicion vive en el cliente), corre las fases (`intro` -> `listen` ->
  `ready` -> `record` -> `upload` -> `playback` por toma -> `wheel`) con `setTimeout`
  propio y es **relay de audio**: cada cliente sube su toma (mu-law crudo, binario por
  socket.io, tope 80 KB) con el puntaje que calculo el mismo, y al abrir la reproduccion
  el server reenvia todas las tomas a todos. Tambien sortea los audios de la
  biblioteca de la comunidad (tabla `imitame_clips` de Supabase, `supabase/imitame.sql`): el
  server no toca la DB, cada cliente le manda los ids en el `mt:join`. Reenvia ademas la
  senalizacion WebRTC del chat de voz del juego (`mt:rtc`). Decide la ruleta (bonus a la mitad de abajo de
  la tabla, sabotajes a la de arriba) y aplica su multiplicador. Ver el `CLAUDE.md` de
  `imitame`.
- `src/games/hotpotato.ts` — `HotPotatoSim`: Papa Caliente. Guarda la **mecha secreta** de
  cada papa (nunca viaja en `hp:state`), quien la tiene, la secuencia de flechas a marcar y
  las eliminaciones. La secuencia la resuelve el cliente sin red y solo viaja el pase
  terminado, con `at` = la hora del server que ese cliente veia al completarlo (offset de
  reloj como PONG, alimentado por `hp:pong`). El server lo toma acotado a
  `[llegada - LAG_COMP_MS, llegada]` y resuelve la explosion `LAG_COMP_MS` (250) despues de
  vencer la mecha, asi un pase que ya venia viajando todavia entra. Estado scopeado por
  **ronda**, igual que Neon Drift. Ver el `CLAUDE.md` de `hot-potato`.
- `src/games/derrumbe.ts` — `DerrumbeSim`: Derrumbe (TNT Run). **Hibrido a proposito**: el
  **piso** (4 capas de 25x25 recortadas en circulo) es autoritativo — cada cliente declara
  sus pasos (`dr:step`), el server programa la caida de cada bloque a `FALL_DELAY_MS`, los
  difunde en el snapshot (`f`) y una vez por segundo manda el tablero entero en hex
  (`doom`) para curar desacuerdos — pero el **movimiento** lo simula cada cliente y el
  server solo lo reenvia a 20 Hz (`dr:snap.p`), como Neon Drift. Reconciliar la fisica
  vertical de un plataformero 3D contra ~150 ms se siente pastoso, y la mecha del bloque
  ya absorbe la latencia. Valida lo que arruinaria la partida ajena: un paso tiene que caer
  a `STEP_TOLERANCE` de la ultima posicion declarada. Tambien decide el orden de
  eliminacion (`dr:dead`), la vuelta de honor del ultimo en pie (`LAP_MS`), el deterioro
  del piso a partir de los 40 s y el tope de 120 s. Al desconectarse, le prende la mecha
  a lo que el jugador tiene bajo los pies (recargar no es un escudo). Ver el `CLAUDE.md` de
  `derrumbe`.
- `src/games/luzroja.ts` — `LuzRojaSim`: Luz Roja, Luz Verde (la cancha de Squid Game). Duenio
  del **semaforo**: sortea cada verde entre tres perfiles de ritmo (rapido / medio / lento) y cada
  rojo, a veces manda un **amague** (`lr:tease`, solo animacion) y corta a los 45 s. La duracion
  del verde viaja en el estado y el cliente reparte la cancion en ella: el ritmo de la cancion ES
  el aviso. **Quien juzga moverse en rojo es cada cliente**, contando su margen desde que VE el
  rojo: juzgarlo en el server castigaria al que tiene peor conexion. El server si valida la
  llegada (`lr:fin` solo vale si la ultima posicion ya cruzo la linea), reenvia las posiciones a
  20 Hz y elimina al que no cruzo a tiempo. Ver el `CLAUDE.md` de `luz-roja`.
- `src/games/pistaloca.ts` — `PistaLocaSim`: Pista Loca (Block Party). Cada ronda son cuatro
  pasos: `dance` (suena la musica), `choose` (se pide un color con su tiempo, que se achica de
  5 s a 1.3 s), `drop` (cae todo lo que no es de ese color) y `reset` (la pista se rearma con otro
  dibujo). El server **arma el dibujo** (manchas, franjas, anillos o mosaico, cada vez con mas
...
- `src/games/marealava.ts` — `MareaLavaSim`: Marea de Lava. Manda la **semilla** de la torre
  ...
  cada uno (quemado / cima, y la mejor altura), valida la cima contra la ultima posicion y, como
  red, elimina a quien quede 2 m por debajo de la lava (asi un desconectado cae solo, sin
  timers). Tope de 120 s. Ver el `CLAUDE.md` de `marea-lava`.
- `src/games/lacuerda.ts` — `LaCuerdaSim`: La Cuerda, **Marea de Lava con dos cuerdas en el lugar
  de la lava, mas el empujon de Pista Loca**. El puente es fijo (no hay semilla); el server lleva
  el reloj y el angulo de cada cuerda es una funcion del tiempo (`ropeAngle`) duplicada en el
  cliente. Lleva el resultado de cada uno (afuera / meta, y el mejor avance), valida la meta
  contra la ultima posicion y, como red, elimina al que declara haber caido al vacio y les pasa
  las cuerdas a los **desconectados** con su ultima posicion. Resuelve el **empujon** (`lc:push`)
  como Pista Loca, pero mas suave y solo sobre el puente: le manda a cada empujado, dirigido, el
  impulso (`lc:shove`) y el vuelo lo simula el empujado. Tope de 90 s: el que no cruzo queda
  afuera. Ver el `CLAUDE.md` de `la-cuerda`.
...
Regla para cualquier juego **de tiempo real arbitrado por el server** (hoy PONG y
Manchon; aplica al que venga, p.ej. rocket-arena si se retoma). Si el cliente dibuja entidades
...
3. **Cliente: reconciliar con acuse de input, nunca con un lerp.** Vale para todo
   ...
   99-103%. **Y las dos cuentas tienen que medir el tiempo igual**, o el pincel
   tiembla unos px en cada snapshot: el snapshot tiene que decir tambien cuanto del
   input acusado ya aplico el server (`a`), el server tiene que aplicar cada input
   con la misma duracion que tuvo en el cliente (un buffer de jitter que lo agenda
   por el reloj del cliente, no a la hora de llegada) y el cliente tiene que predecir
   con el vector enviado y el reloj del cuadro. Ver "El pincel propio tiembla" en el
   `CLAUDE.md` de `paint-turf` (medido: de 33-61 a 1-6 px/s de desvio).
...
cadencia mas baja, pero la carrera es identica. Bomba Palabra, Cadena de Palabras, Basta,
Impostor, Telefono Cortado, Manchon, Imitame, Papa Caliente, Derrumbe, Luz Roja, Pista Loca, Marea de Lava y La Cuerda **no** pueden: existen por el server (en Bomba/Cadena porque el
diccionario vive ahi; en Basta e Impostor porque el server arbitra las fases, los votos y el
puntaje —y en Impostor tambien reparte los roles privados; en Telefono Cortado porque es el
unico que conoce las frases secretas y valida las adivinanzas; en Manchon porque el
tablero es estado compartido y sin arbitro cada pantalla tendria otro ganador; en Imitame porque
el server corre las fases y reparte las tomas de audio; en Papa Caliente porque la mecha tiene que
ser secreta y solo el server puede guardarla; en Derrumbe por lo mismo que en Manchon, con el piso; en Luz Roja porque el semaforo tiene que ser
el mismo para todos y solo el server lo puede sortear; en Pista Loca porque el dibujo, el color
y el ritmo tienen que ser los mismos para todos; en Marea de Lava porque la torre y la lava
tienen que ser las mismas para todos; en La Cuerda porque el reloj de las cuerdas tiene que ser el
mismo para todos), asi
...
- `resolveGameServerUrl()` — la URL a la que conectarse, con caida al respaldo. **La
  llaman los quince juegos server-side** en su `connect()` (`word-bomb`, `word-chain`,
  `basta`, `impostor`, `telefono-cortado`, `pong`, `car-race`, `paint-turf`, `imitame`,
  `hot-potato`, `derrumbe`, `luz-roja`, `pista-loca`, `marea-lava`, `la-cuerda`), que por eso es `async` y
...
### El toque de arranque no puede colgar del canvas (el bug del overlay)

La otra mitad del patron: **el arranque tiene que entrar tambien por un toque**, porque en un celular no hay Enter. Y el gotcha es siempre el mismo, medido en 14 juegos (`block-paddle`, `circuit-breaker`, `penalty-keeper`, `reaction-time`, `rhythm-tap`, `shell-game`, `snake`, `stack-tower`, `timing-bar`, `wordle`, mas `click-the-number`, `sliding-puzzle`, `lights-out` y `tower-of-hanoi`, que ademas tienen un selector: ver abajo): el `pointerdown` estaba registrado **sobre el canvas**, y la pantalla de inicio / game over es un **overlay que lo tapa**. En desktop no se nota (arrancas con Enter y listo), pero en el telefono el toque moria en el overlay y el juego **no tenia forma de empezar** — quedaba invariablemente en la pantalla de inicio. Varios de esos `CLAUDE.md` ya prometian "Enter or tap to start": el listener existia, simplemente nunca recibia el evento.

Asi que el listener de arranque va **sobre el `container`** (que contiene al overlay; es lo que hacen `flappy-bird` y `stack-tower`) o **directo sobre el overlay** (`timberman`, `memory-match` con su `onOverlayTap`). Nunca sobre `this.canvas` ni sobre `renderer.domElement`.

Dos detalles que hacen que colgarlo del container sea seguro:

- **Usar `pointerdown`, no `click` ni `touchstart`.** El `LeaderboardPanel` compartido hace `stopPropagation` de **`pointerdown`** sobre su raiz (`src/shared/LeaderboardPanel.ts`), asi que tocar el campo del nombre o el boton de enviar no dispara el arranque. Con `click` no estas cubierto por esa proteccion, y con `mousedown` + `touchstart` juntos te comes el doble disparo en movil que obliga a andar con `preventDefault`.
- **El handler tiene que filtrar por estado.** Como ahora el listener ve los toques de toda la pantalla (incluidos los del gameplay: las teclas de `wordle`, los vasos de `shell-game`), la accion de arranque solo puede correr en los estados de menu (`ready` / `dead` / `gameOver` / `roundEnd`). Todos los juegos ya tenian ese `switch`; es lo que hace el cambio inofensivo.

**Jugable en celular (`mobile` en el `meta.ts`).** Para declarar `mobile: true` hacen falta las dos mitades: que se pueda **arrancar** con un toque (lo de arriba) y que se pueda **jugar** sin teclado fisico (puntero/tactil, botones en pantalla, o un teclado propio en pantalla como el de `wordle`). Los unicos tres `false` de hoy son `mecano`, `typing-race` y `hackerman`: escuchan `window keydown` sin ningun `<input>` en el DOM, asi que en un telefono el teclado virtual no aparece nunca y no hay con que jugarlos — eso no se arregla con un listener, pide rediseñar el input.

Como se verifica (es lo que se uso para auditar el roster): Playwright con `hasTouch: true` a 390x844, `page.touchscreen.tap()` en el medio de la pantalla, y mirar si aparece el `.countdown`. Si el tap no arranca pero `Enter` si, el juego tiene este bug. **Tocar en un solo punto fijo no alcanza**: ese punto puede caer justo sobre un control propio de la pantalla de inicio (el selector de circuitos de `car-race`) o sobre el panel de ranking, que corta la propagacion a proposito, y el juego se reporta roto estando sano. Probar varias alturas y dar por bueno el juego si arranca en alguna, que es lo que hace una persona con el dedo.

**Los juegos con selector (tamano / discos) son un caso aparte, y son los que se escapan de la auditoria.** `click-the-number`, `sliding-puzzle`, `lights-out` y `tower-of-hanoi` muestran botones de opcion (1-9 / 3x3 / 3 discos) en la pantalla de inicio, pero su handler **solo guarda la opcion** (`handleSelectSize` / `handleSelectDiscs` hacen `this.size = size` y nada mas): el unico arranque era `Enter`. En el telefono se podia elegir el tamano y despues no habia como empezar. El toque va sobre el container como en el resto, pero **exento el selector** (`e.target.closest(".overlay__size-selector")` / `".overlay__disc-selector"` devuelve temprano), asi tocar una opcion la elige y deja ver su record, y el toque en cualquier otro lado arranca. El hint de la pantalla de inicio tiene que decirlo ("toca la pantalla o presiona ENTER"): un toque que arranca desde cualquier lado menos los botones no se adivina.

**Al verificar, la senal tiene que ser el `.countdown`, nunca "cambio el texto de la pantalla".** Es el error que dejo pasar esos cuatro: tocar `1-9` marca el boton como activo y repinta el record de ese tamano, con lo cual el `innerText` cambia y una heuristica de "cambio algo" lo cuenta como arranque. Lo mismo pasa al reves con los overlays que se ocultan con `opacity: 0` en vez de `display: none` (`mini-frogger`): `innerText` sigue leyendo el cartel transparente, y un juego que arranco perfecto se reporta como roto. Medir el `.countdown` (o, en los que no lo tienen, el estado real del overlay) es lo unico que no miente.
