...
Como se verifica (es lo que se uso para auditar el roster): Playwright con `hasTouch: true` a 390x844, `page.touchscreen.tap()` en el medio de la pantalla, y mirar si aparece el `.countdown`. Si el tap no arranca pero `Enter` si, el juego tiene este bug.
