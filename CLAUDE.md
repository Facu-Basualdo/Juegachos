...
vive solo en el server, que ademas compensa la latencia de cada pase) **y Neon Drift** (`car-race`, **relay** de
...
- `src/protocol.ts` — tipos de mensajes (por juego: `wb:*` de Bomba Palabra,
  ...
  `tc:*` de Telefono Cortado, `pt:*` de Manchon, `mt:*` de Imitame, `hp:*` de Papa Caliente). **Se duplican en el cliente**
...
Impostor, Telefono Cortado, Manchon, Imitame y Papa Caliente **no** pueden: existen por el server (en Bomba/Cadena porque el
...
ser secreta y solo el server puede guardarla), asi
...
- `resolveGameServerUrl()` — la URL a la que conectarse, con caida al respaldo. **La
  ...
  `hot-potato`), que por eso es `async` y
