...
La otra mitad del patron: **el arranque tiene que entrar tambien por un toque**, porque en un celular no hay Enter. Y el gotcha es siempre el mismo, medido en 10 juegos a la vez (`block-paddle`, `circuit-breaker`, `penalty-keeper`, `reaction-time`, `rhythm-tap`, `shell-game`, `snake`, `stack-tower`, `timing-bar`, `wordle`): el `pointerdown` estaba registrado **sobre el canvas**, y la pantalla de inicio / game over es un **overlay que lo tapa**. En desktop no se nota (arrancas con Enter y listo), pero en el telefono el toque moria en el overlay y el juego **no tenia forma de empezar** — quedaba invariablemente en la pantalla de inicio. Varios de esos `CLAUDE.md` ya prometian "Enter or tap to start": el listener existia, simplemente nunca recibia el evento.
