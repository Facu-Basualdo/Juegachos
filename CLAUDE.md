...
- `src/protocol.ts` — tipos de mensajes (por juego: `wb:*` de Bomba Palabra,
  ...
  `tc:*` de Telefono Cortado, `pt:*` de Manchon, `mt:*` de Imitame, `hp:*` de Papa Caliente, `dr:*` de Derrumbe, `lr:*` de Luz Roja, Luz Verde). **Se duplican en el cliente**
...
Impostor, Telefono Cortado, Manchon, Imitame, Papa Caliente, Derrumbe y Luz Roja **no** pueden: existen por el server (en Bomba/Cadena porque el
...
el mismo para todos y solo el server lo puede sortear), asi
...
- `resolveGameServerUrl()` — la URL a la que conectarse, con caida al respaldo. **La
  llaman los doce juegos server-side** en su `connect()` (`word-bomb`, `word-chain`,
  `basta`, `impostor`, `telefono-cortado`, `pong`, `car-race`, `paint-turf`, `imitame`,
  `hot-potato`, `derrumbe`, `luz-roja`), que por eso es `async` y
