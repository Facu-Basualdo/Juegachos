...
semilla de la torre y lleva el reloj de la lava) **y Neon Drift** (`car-race`, **relay** de
...
- `src/protocol.ts` — tipos de mensajes (por juego: `wb:*` de Bomba Palabra,
  ...
  `tc:*` de Telefono Cortado, `pt:*` de Manchon, `mt:*` de Imitame, `hp:*` de Papa Caliente, `dr:*` de Derrumbe, `lr:*` de Luz Roja, Luz Verde, `pl:*` de Pista Loca, `ml:*` de Marea de Lava). **Se duplican en el cliente**
...
Impostor, Telefono Cortado, Manchon, Imitame, Papa Caliente, Derrumbe, Luz Roja, Pista Loca y Marea de Lava **no** pueden: existen por el server (en Bomba/Cadena porque el
...
tienen que ser las mismas para todos), asi
...
- `resolveGameServerUrl()` — la URL a la que conectarse, con caida al respaldo. **La
  llaman los catorce juegos server-side** en su `connect()` (`word-bomb`, `word-chain`,
  `basta`, `impostor`, `telefono-cortado`, `pong`, `car-race`, `paint-turf`, `imitame`,
  `hot-potato`, `derrumbe`, `luz-roja`, `pista-loca`, `marea-lava`), que por eso es `async` y
