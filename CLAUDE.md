...
las fases y hace de relay de las tomas de voz) **y Neon Drift** (`car-race`, **relay** de
...
Impostor, Telefono Cortado, Manchon e Imitame **no** pueden: existen por el server (en Bomba/Cadena porque el
...
el server corre las fases y reparte las tomas de audio), asi
...
- `resolveGameServerUrl()` — la URL a la que conectarse, con caida al respaldo. **La
  llaman los nueve juegos server-side** en su `connect()` (`word-bomb`, `word-chain`,
  `basta`, `impostor`, `telefono-cortado`, `pong`, `car-race`, `paint-turf`, `imitame`), que por eso es `async` y
